...
* Using the layer called "mix_31" (see previous exercise), create a variable called "nb_rest_10000inhab" which corresponds to the number of restaurants per 10,000 inhabitants in each territory
...
Diverses remarques/ questions à régler concernant cet exercice : 

* J'ai remplacé bks - 1 par bks dans fufun2 mais on pourra en discuter ensemble car je n'ai pas compris leur utilité (idem j'ai enlevé rasdens <- rasdens + 1)
Les légendes des cartes générées par fufun et fufun2 ne sont pas pareil (légende de fufun2 plus tassée), impossible également de comprendre pourquoi => à discuter
* J'ai rajouté des st_intersection dans la fonction fufun pour rendre les contours plus nets, surtout pour la fonction fufun qui remplissait sinon sur tout le carré de grid. Je n'arrive pas par contre à faire la même chose pour fufun2 car rasdens est de type raster et non polygon...
* J'ai un warning généré par fufun2 que j'arrive pas à régler :  attribute variables are assumed to be spatially constant throughout all geometries
* On pourra améliorer la représentation de la question 1 une fois que tu auras davantage développé la partie sur mapview du cours
* La légend des cartes (0.5 km) est effacée car trop à droite. Comment corriger cela ? 
```

1. Using the data contained in "sir_31" used previously, map the more than 4,000 restaurants of department 31 with the {mapview} package. Try using different parameters to customize your map.
...
2. Apply the fufun2 function below on the SIRENE data and on the iris of the department 31. Use a 2km cellsize. What observation can you make concerning the readability of the map?
...
```{block, box.title=" Information",box.type = "advice"}
Use the map layer "epci_31" from previous exercises and the {cartogram} package. 
```
...
```{block, box.title=" Information",box.type = "advice"}
Use the map layer "com_31" from previous exercises and the {linemap} package. 
```
