...
We would like here to design EPCI maps that combine the number of restaurants and the number of restaurants per 10000 inhabitants. 
...
```{block, box.title = "3", box.body = list(fill = "white"), box.icon = "fa-star"}
What other solution could we use to display these two variable on the same map? 
```
...
```{block, box.title = "4", box.body = list(fill = "white"), box.icon = "fa-star"}
Display the number of restaurants and the number of restaurants per 10,000 inhabitants at the commune and EPCI scales. The two maps displayed side by side should be as much comparable as possible.
```
...
# Define breaks for communes (we will use the same breaks for both maps)
...
```{block, box.title = "3", box.body = list(fill = "white"), box.icon = "fa-star"}
Simply use the `plot` instruction to plot the grid. Then, using the `choroLayer` function of the `cartography` package, plot the number of restaurants per kilometer (called ‘n’) in each box of the grid. What observation can you make concerning the readability of the map?
```
