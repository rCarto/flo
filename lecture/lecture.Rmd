...
Nonethless they can be really useful for exploratory data analisys. 
...
Create a small function that build a regular grid and count points in each cells. 
```{r, cache=TRUE}
pt_in_grid <- function(feat, adm, cellsize = 50){
  ...
  # keep only cells that intersects adm
...
Create a function that plot a map
...
Define a function that compute smoothed restaurant density (KDE method).
...
compute_kde <- function(feat, adm, title, sigma = 100, res = 50){
  ...
  # im to raster (+ proj & km2)
  result <- raster(dens, crs = st_crs(adm)[[2]]) * 1000000
  return(result)
}
```

Define a function that map the results
```{r, cache=TRUE}
